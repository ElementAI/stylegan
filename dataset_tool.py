...
class TFRecordExporter:
    def __init__(self,
        ...
        self.tfr_prefix = os.path.join(self.tfrecord_dir,
                                       os.path.basename(self.tfrecord_dir))
    ...
    def choose_shuffled_order(
            self):  # Note: Images and labels must be added in shuffled order.
...
class ThreadPool(object):
    ...
    def process_items_concurrently(self,
        ...
        def retire_result():
            ...
            while retire_idx[0] < len(results) and results[
                    retire_idx[0]] is not None:
...
def create_mnistrgb(tfrecord_dir,
                    mnist_dir,
                    num_images=1000000,
                    random_seed=123):
    print('Loading MNIST from "%s"' % mnist_dir)
...
def create_cifar10(tfrecord_dir, cifar10_dir):
    ...
    for batch in range(1, 6):
        with open(os.path.join(cifar10_dir, 'data_batch_%d' % batch),
                  'rb') as file:
...
def create_svhn(tfrecord_dir, svhn_dir):
    ...
    for batch in range(1, 4):
        with open(os.path.join(svhn_dir, 'train_%d.pkl' % batch),
                  'rb') as file:
            data = pickle.load(file, encoding='latin1')
        ...
        with TFRecordExporter(tfrecord_dir, max_images) as tfr:
            for _idx, (_key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    img = img[(img.shape[0] - crop) // 2:(
                        img.shape[0] + crop) // 2, (img.shape[1] - crop) //
                              2:(img.shape[1] + crop) // 2]
...
def create_lsun_wide(tfrecord_dir,
                     lmdb_dir,
                     width=512,
                     height=384,
                     max_images=None):
    ...
    with lmdb.open(lmdb_dir, readonly=True).begin(write=False) as txn:
        ...
        if max_images is None:
            max_images = total_images
        with TFRecordExporter(
                tfrecord_dir, max_images, print_progress=False) as tfr:
            for idx, (_key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    img = img[(img.shape[0] - ch) // 2:(img.shape[0] + ch) //
                              2]
                    img = PIL.Image.fromarray(img, 'RGB')
                    ...
                    canvas[:, (width - height) // 2:(width + height) //
                           2] = img
                    tfr.add_image(canvas)
                    print(
                        '\r%d / %d => %d ' % (idx + 1, total_images,
                                              tfr.cur_images),
                        end='')
...
def create_from_images_and_json(tfrecord_dir, image_dir, json_dir,
                                cat_mapping_file, shuffle):
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    if len(image_filenames) == 0:
        error('No input images found')

    json_dir_full = os.path.join(image_dir, json_dir)
    print("Json dir", json_dir_full)
    ...
    cat_mapping_dict = json.load(os.path.join(image_dir, cat_mapping_file))
    print("Cat mapping dict")
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            ...
            json_filename = os.path.join(
                json_dir_full,
                "{}.json".format(image_filenames[order[idx]].split('_')[0]))
            metadata = json.load(json_filename)
            category = cat_mapping_dict[metadata['category']]
            tfr.add_label(category)
    ...
    with h5py.File(hdf5_filename, 'r') as hdf5_file:
        ...
        with TFRecordExporter(tfrecord_dir, hdf5_image.shape[0]) as tfr:
            order = tfr.choose_shuffled_order() if shuffle else np.arange(
                hdf5_image.shape[0])
            for idx in range(order.size):
                shuffled_im = np.tranpose(hdf5_image[order[idx]], (2, 0, 1))
...
def execute_cmdline(argv):
    ...
    def add_command(cmd, desc, example=None):
        epilog = 'Example: %s %s' % (prog,
                                     example) if example is not None else None
    ...
    p.add_argument(
        'tfrecord_dir_b', help='Directory containing second dataset')
    p.add_argument(
        '--ignore_labels',
        help='Ignore labels (default: 0)',
        type=int,
        default=0)
    ...
    p.add_argument('lmdb_dir', help='Directory containing LMDB database')
    p.add_argument(
        '--width', help='Output width (default: 512)', type=int, default=512)
    p.add_argument(
    ...
    p.add_argument(
        '--cy',
        help='Center Y coordinate (default: 121)',
        type=int,
        default=121)

    p = add_command('create_from_images',
